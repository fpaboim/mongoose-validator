  ...
  return function validator(val) {
    ...
    if (Array.isArray(val)) {
      for (let i=0; i <= val.length; i++) {
        let validatorArgs = [val[i]].concat(args)

        if ((passIfEmpty && (is.empty(val[i]) || is.nil(val[i]))) || is.undef(val[i])) {
          return true
        }
        if (fn.apply(this, validatorArgs)) {
          return true
        }
      }
      return false
    }
    let validatorArgs = [val].concat(args)
